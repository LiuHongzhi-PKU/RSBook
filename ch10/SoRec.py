...
class SoRec(torch.nn.Module):
    def __init__(self, n_users, n_items, social_user, n_factors=10, lr=0.01, lambda_1=10, lambda_2=0.001,sparse=False, device=torch.device("cuda")):
        ...
        self.social_embeddings = nn.Embedding(self.n_items, self.n_factors, sparse=sparse)
    ...
    def social_forward(self, users, users_2):
        ...
        ues = self.user_embeddings(users)
        ues_2 = self.item_embeddings(users_2)
                ...
                for batch_idx, ((row, col), val) in pbar:
                    ...
                    for i in range(row.shape[0]):
                        ...
                        if len(self.social_user[int(row[i])])>0:
                            sim_user += random.sample(self.social_user[int(row[i])],1)
                            sim_val.append(1/len(self.social_user[int(row[i])]))
                            row_2.append(row[i])
                    # sim_val = [x/len(self.social_user[int(row[i])]) for x in sim_val]
                    sim_user=torch.tensor(sim_user).long().to(self.device)
                    row_2 = torch.tensor(row_2).long().to(self.device)
                    sim_val = torch.tensor(sim_val).float().to(self.device)
                    sim_preds = self.social_forward(row_2, sim_user)
...
if __name__ == '__main__':
    ...
    model = SoRec(input_size[0],input_size[1],input_size[2])
    model.fit(loader,20)
    torch.save(model.state_dict(), 'SoRec.pkl')