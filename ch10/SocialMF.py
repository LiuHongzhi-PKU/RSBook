...
class SocialMF(torch.nn.Module):
    def __init__(self, n_users, n_items, social_user, n_factors=10, lr=0.01, lambda_1=5, lambda_2=0.001,sparse=False, device=torch.device("cuda")):
                ...
                for batch_idx, ((row, col), val) in pbar:
                    ...
                    for i in range(row.shape[0]):
                        ...
                        if len(self.social_user[int(row[i])])>0:
                            ...
                            tmp_emb = torch.zeros(self.n_factors, dtype=torch.float32).to(self.device)
                            ...
                            sim_embs.append(tmp_emb)
                            row_2.append(row[i])
                    row_2 = torch.tensor(row_2).long()
                    sim_embs = torch.stack(sim_embs)
                    user_embs = self.user_embeddings(row_2)
                    loss += self.lambda_1 * ((user_embs - sim_embs)*(user_embs - sim_embs)).sum()
...
if __name__ == '__main__':
    ...
    model = SocialMF(input_size[0],input_size[1],input_size[2],device="cuda")
    model.fit(loader,10)
    torch.save(model.state_dict(), 'SocialMF.pkl')