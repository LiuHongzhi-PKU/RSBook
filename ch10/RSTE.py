...
class RSTE(torch.nn.Module):
    def __init__(self, n_users, n_items, social_user, n_factors=10, lr=0.01, alpha=0.4, lambda_2=0.001, sparse=False, device=torch.device("cuda")):
                ...
                for batch_idx, ((row, col), val) in pbar:
                    ...
                    for i in range(row.shape[0]):
                        ...
                        if len(self.social_user[int(row[i])])>0:
                            tmp_sum = torch.zeros(1, dtype=torch.float32).to(self.device)
                            for user_2 in self.social_user[int(row[i])]:
                                user_2=torch.tensor([user_2]).long().to(self.device)
                                ipt2=torch.tensor([col[i]]).long().to(self.device)
                                tmp = self.forward(user_2,ipt2)
                                tmp_sum +=tmp/len(self.social_user[int(row[i])])
                            preds[i]=self.alpha*preds[i]+(1-self.alpha)*tmp_sum
...
if __name__ == '__main__':
    ...
    model = RSTE(input_size[0],input_size[1],input_size[2],device="cuda")
    model.fit(loader,10)
    torch.save(model.state_dict(), 'RSTE.pkl')