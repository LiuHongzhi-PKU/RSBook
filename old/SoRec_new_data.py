...
class LFM(torch.nn.Module):
    ...
    def social_forward(self, users, users_2):
        ues = self.user_embeddings(users)
        ues_2 = self.social_embeddings(users_2)

        preds = self.user_biases(users)
        preds += self.social_biases(users_2)
        preds += (self.dropout(ues) * self.dropout(ues_2)).sum(dim=1, keepdim=True)
                ...
                # for batch_x, batch_y in loaders[phase]:
                    ...
                    for i in range(row.shape[0]):
                        ...
                        if len(self.social_user[int(row[i])])>0:
                            ...
                            sim_val.append(1)
                            row_2.append(row[i])
                    sim_user=torch.tensor(sim_user).long()
                    row_2 = torch.tensor(row_2).long()
                    sim_val = torch.tensor(sim_val).float()
                    sim_preds = self.social_forward(row_2, sim_user)
                    loss = nn.MSELoss(reduction='sum')(preds, val)
                    loss += nn.MSELoss(reduction='sum')(sim_preds, sim_val)
