...
class heteMF(torch.nn.Module):
    def __init__(self, n_users, n_items, hinSim, hinSimI, metaPath, n_factors=10, lr=0.01, lambda_0=0.001,lambda_I=1, device=torch.device("cpu")):
        ...
        self.W_I=nn.Embedding(len(self.path2id), 1) # 可能需要注册参数
            ...
            for phase in ['train', 'valid']:
                ...
                # for batch_x, batch_y in loaders[phase]:
                    ...
                    for p in self.metaPath['PI']:
                        ...
                        # print(hin_pow.shape)
                        loss_PI = loss_PI + (self.lambda_I * torch.exp(self.W_I(self.path2id[p])) * hin_value * hin_pow).sum()
                    ...
                    for p in self.metaPath['PU']:
                        ...
                        # print(hin_pow.shape)
                        loss_PI = loss_PI + (
                                    self.lambda_U * torch.exp(self.W_I(self.path2id[p])) * hin_value * hin_pow).sum()
                        WI_sum = WI_sum + torch.exp(self.W_I(self.path2id[p])).squeeze()
                    ...
                    WI_sum = 0
                    loss_PI = 0
                    for p in self.metaPath['PUI']:
                        hin_value = self.hinSim[p][batch_U]  # U*K->B*K  前K个的取值
                        hin_index = self.hinSimI[p][batch_U]  # U*K -> B*K 前K个的下标

                        y_pred = self.forward(batch_U, hin_index)
                        loss_PI=loss_PI+torch.exp(self.W_I(self.path2id[p]))*nn.MSELoss(reduction='sum')(y_pred, hin_value)
...
if __name__ == '__main__':
    ...
    model = heteMF(input_size[0],input_size[1], input_size[2], input_size[3],input_size[4])
    model.fit(loader,10)
