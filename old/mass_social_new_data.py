...
def getDataLoader(data_path, batch_size=32):
    ...
    # 按照1:9划分数据集
    for (user, item, record) in data_df.itertuples(index=False):
...
class userCF():
    def __init__(self, n_users, n_items,social_user, K=20,N=10):
        self.K = K  # 近邻数
        self.N = N  # 物品推荐数
    ...
    def fit(self, train_data, Step=6):
        ...
        # user_item={1:{1:1,2:1},2:{1:1,3:1},3:{2:1,3:1},4:{3:1,4:1},5:{2:1,4:1}}
        # self.social_user={1:set([2,3,5]),2:set([1]),3:set([1,4,5]),4:set([3]),5:set([1,3])}
        ...
        for u in tqdm(list(user_item)):
            item_cnt=defaultdict(int)
            user_cnt=defaultdict(int)
            social_cnt = defaultdict(int)
            train_user = set(user_item)
            for user in list(self.social_user):
                self.social_user[user]=self.social_user[user] & train_user
                ...
                if k %3 ==1:
                    ...
                    for user in user_cnt:
                        for item in user_item[user]:
                            item_cnt[item]+=user_cnt[user]/(len(user_item[user])+len(self.social_user[user]))
                        for user_2 in self.social_user[user]:
                            # pass
                            social_cnt[user_2]+=user_cnt[user]/(len(user_item[user])+len(self.social_user[user]))
                    user_cnt=defaultdict(int)
                    # pass
                    ...
                    # 用户扩散项目（第三步）
                    for user in social_cnt:
                        # pass
                        # print(len(user_item[user]))
                        # # print(len(user_item[user]))
                        for item in user_item[user]:
                        #     a=2
                            item_cnt[item] += social_cnt[user] / len(user_item[user])
                    social_cnt = defaultdict(int)
            # print(len(visit_cnt))
            # item_cnt
...
if __name__ == '__main__':
    ...
    model = userCF(input_size[0],input_size[1],input_size[2])
