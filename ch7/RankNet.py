...
def rankNet_loss(pos_preds,neg_preds):
        ...
        for epoch in range(epochs):
            ...
            for phase in ['train', 'valid']:
                ...
                for batch_idx, ((users, items), ratings) in pbar:
                    ...
                    loss = rankNet_loss(pos_preds,neg_preds)
...
if __name__ == '__main__':
    model = RankNet("../data/ml-100k/u.data", n_factors=opt.n_factors, lr=opt.lr, topn=opt.topn,
                weight_decay=opt.weight_decay, batch_size=opt.batch_size)
    model.fit(opt.epochs)
# precisioin=0.2207	recall=0.1038	coverage=0.4055