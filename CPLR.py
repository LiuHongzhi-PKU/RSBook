...
def getDataLoader(data_path, batch_size=32, K=10):
    ...
    for phase in ['train', 'valid']:
        ...
        support[phase] = getSupport(user_sim, user_item, K)

        loader[phase]=data.DataLoader(
            CPLR_Interactions(df[phase], n_items, user_sim, K), batch_size=batch_size, shuffle=(phase == 'train'))
    ...
    return (n_users, n_items, support), loader
...
class CPLR(torch.nn.Module):
    def __init__(self, n_users, n_items, support, n_factors=20, alpha=1, beta=1, gama=1, lr=0.1, weight_decay=0.005,device=torch.device("cpu"),
